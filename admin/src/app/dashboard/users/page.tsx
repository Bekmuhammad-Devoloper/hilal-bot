      ...
      <div className="flex flex-wrap justify-between items-center gap-3">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-0.5">Jami: <span className="font-semibold text-indigo-600">{data.total || 0}</span></p>
          </div>
        </div>
        <div className="relative">
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Qidirish..." className="w-72 pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 outline-none transition-all" />
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100">
                ...
                <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Obuna</th>
                <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{"To'lov turi"}</th>
                ...
                <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    {u.activeSubscription ? (
                      <div className="space-y-1">
                        <p className="text-[11px] font-semibold text-indigo-600">{u.activeSubscription.planName}</p>
                        <div className="flex items-center gap-1.5">
                          <svg className="w-3 h-3 text-emerald-500 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg>
                          <span className="text-[10px] text-slate-500">{new Date(u.activeSubscription.startDate).toLocaleDateString("uz-UZ")}</span>
                          <span className="text-[10px] text-slate-300">{"\u2192"}</span>
                          <span className={`text-[10px] font-medium ${new Date(u.activeSubscription.endDate) < new Date() ? "text-red-500" : "text-emerald-600"}`}>{new Date(u.activeSubscription.endDate).toLocaleDateString("uz-UZ")}</span>
                        </div>
                      </div>
                    ) : (
                      <span className="text-[11px] text-slate-300 italic">{"Obuna yo'q"}</span>
                    )}
                  </td>
                  <td className="px-5 py-3.5">
                    {u.lastPaymentMethod ? (
                      <div className="flex items-center gap-2">
                        {u.lastPaymentMethod === "click" ? (
                          <img src="/click-01.png" alt="Click" className="h-5 object-contain" />
                        ) : u.lastPaymentMethod === "payme" ? (
                          <img src="/payme-01.png" alt="Payme" className="h-5 object-contain" />
                        ) : (
                          <span className="px-2 py-0.5 rounded-lg text-[10px] font-semibold bg-slate-50 text-slate-500 uppercase">{u.lastPaymentMethod}</span>
                        )}
                      </div>
                    ) : (
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`px-2 py-0.5 rounded-lg text-[10px] font-semibold ${u.isBlocked ? "bg-red-50 text-red-500" : "bg-emerald-50 text-emerald-500"}`}>{u.isBlocked ? "BLOCKED" : "AKTIV"}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => toggleAdmin(u.id, !u.isAdmin)} title={u.isAdmin ? "Admin olib tashlash" : "Admin qilish"} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${u.isAdmin ? "bg-amber-50 text-amber-500 hover:bg-amber-100" : "bg-slate-50 text-slate-400 hover:bg-slate-100"}`}>
                      ...
                      <button onClick={() => toggleBlock(u.id, !u.isBlocked)} title={u.isBlocked ? "Blokdan chiqarish" : "Bloklash"} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${u.isBlocked ? "bg-red-50 text-red-500 hover:bg-red-100" : "bg-emerald-50 text-emerald-500 hover:bg-emerald-100"}`}>
                      ...
                      <button onClick={() => setMsgModal(u)} title="Xabar yuborish" className="w-8 h-8 rounded-lg flex items-center justify-center bg-blue-50 text-blue-500 hover:bg-blue-100 transition-colors">
                      ...
                      <a href={`/app?user=${u.telegramId}`} target="_blank" rel="noopener noreferrer" title="User panelini ko'rish" className="w-8 h-8 rounded-lg flex items-center justify-center bg-violet-50 text-violet-500 hover:bg-violet-100 transition-colors">
      ...
      {data.totalPages > 1 && (
        <div className="flex gap-1.5 justify-center">
          {Array.from({ length: data.totalPages }, (_, i) => (
            <button key={i} onClick={() => setPage(i + 1)} className={`w-9 h-9 rounded-xl text-sm font-medium transition-all ${page === i + 1 ? "bg-gradient-to-r from-violet-500 to-indigo-600 text-white shadow-md shadow-indigo-500/25" : "bg-white text-slate-500 border border-slate-200 hover:bg-slate-50"}`}>{i + 1}</button>
          ))}
        </div>
      )}
      {msgModal && (
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-slate-100">
            <div className="flex items-center gap-3 mb-4">
              {msgModal.photoUrl ? (
                ...
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center text-blue-600 font-bold">{msgModal.firstName?.charAt(0)?.toUpperCase() || "?"}</div>
              )}
              <div>
